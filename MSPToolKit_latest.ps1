...
"@
                ...
                $result = [System.Windows.Forms.MessageBox]::Show(
                    "Would you like to save this onboarding plan to the desktop?",
                ...
                if ($result -eq [System.Windows.Forms.DialogResult]::Yes) {
                    ...
                    $filename = "Onboarding_Plan_$($nameBox.Text.Trim() -replace '[\\/:*?"<>|]', '_')_$timestamp.txt"
                    $path = "$env:USERPROFILE\Desktop\$filename"
                    $onboardReport | Out-File -FilePath $path -Encoding UTF8
                    Log "Onboarding plan saved to: $path" "PASS"
                }
            }
        } catch {
...
"@
                ...
                # Offer to save the report
                $result = [System.Windows.Forms.MessageBox]::Show(
                    "Would you like to save this offboarding plan to the desktop?",
                    "Save Offboarding Plan", 
                    [System.Windows.Forms.MessageBoxButtons]::YesNo, 
                    [System.Windows.Forms.MessageBoxIcon]::Question
                )
                ...
                if ($result -eq [System.Windows.Forms.DialogResult]::Yes) {
                    $timestamp = Get-Date -Format "yyyyMMdd"
                    $filename = "Offboarding_Plan_$($nameBox.Text.Trim() -replace '[\\/:*?"<>|]', '_')_$timestamp.txt"
                    $path = "$env:USERPROFILE\Desktop\$filename"
                    $offboardReport | Out-File -FilePath $path -Encoding UTF8
                    Log "Offboarding plan saved to: $path" "PASS"
                }
            }
        } catch {
...
# Add remaining sections and buttons here...
# (In a complete script, all functions would be implemented)
...
function Fix-MappedDrives {
    Log "Checking mapped network drives and attempting reconnects..." "INFO"
    try {
        ...
        foreach ($m in $maps) {
            ...
            if (-not (Test-Path ($drive + ':\'))) {
                Log ("Attempting to remap {0} -> {1}" -f $drive, $root) "INFO"
                try {
                ...
                Log (("Drive {0} is connected" -f $drive)) "INFO"
            }
        }
    } catch {
        Log "Mapped drive fix error: $_" "FAIL"
    }
}

function Reset-NetworkStack {
    if (-not (Show-Confirmation -Message "Network stack reset will modify networking components and may disconnect remote sessions. Continue?")) { return }
    Log "Resetting network stack (winsock, TCP/IP)..." "INFO"
    try {
        ...
        Log "Network stack reset commands executed. A reboot may be required." "PASS"
    } catch {
        Log "Network reset error: $_" "FAIL"
    }
}

# Outlook helpers
function Fix-OutlookProfile {
...
function Remove-OfflinePrinters {
    Log "Removing offline/ghost printers where possible..." "INFO"
    try {
        $printers = Get-WmiObject Win32_Printer | Where-Object { $_.WorkOffline -eq $true -and $_.Default -ne $true }
        foreach ($p in $printers) {
            Log ("Removing printer: {0}" -f $p.Name) "INFO"
            try { $p.Delete() | Out-Null; Log ("Removed {0}" -f $p.Name) "PASS" } catch { Log ("Failed to remove {0}: $_" -f $p.Name) "WARN" }
        }
    } catch {
        Log "Remove printers error: $_" "FAIL"
    }
}
...
# Create buttons for the new common cases
...
$btnOutlookFix = New-Button -Text "Fix Outlook Profile & Restart" -Name "btnOutlookFix" -BackColor ([System.Drawing.Color]::FromArgb(60,70,80)) -ToolTipText "Backup OST files and restart Outlook to recreate profile" -Action { Fix-OutlookProfile }
...
function Check-O365LicenseStatus {
    param([string]$UserUPN)
    if (-not $UserUPN) {
        Log "No user specified for O365 license check. Provide a UPN when invoking this function." "WARN"
        Log "Example usage: Check-O365LicenseStatus -UserUPN 'user@domain.com' (requires prior MSOnline connection)." "INFO"
        return
    }

    if (-not (Get-Module -ListAvailable -Name MSOnline)) {
        Log "MSOnline module not installed. Install-Module MSOnline to enable license checks." "WARN"
        return
    }

    try {
        # Do NOT attempt an interactive Connect-MsolService here. If the module/session is already available,
        # Get-MsolUser will succeed; otherwise instruct the operator to connect manually.
        try {
...
# Buttons for the new MSP tools
$btnBitLocker = New-Button -Text "BitLocker: Recovery Info" -Name "btnBitLocker" -BackColor ([System.Drawing.Color]::FromArgb(60,70,80)) -ToolTipText "Show BitLocker protector/recovery info" -Action { Get-BitLockerRecoveryInfo }
$btnBitLocker.Location = New-Object System.Drawing.Point(10, $yPosition)
$buttonContainer.Controls.Add($btnBitLocker)
$yPosition += 47

$btnO365 = New-Button -Text "Office365: License Check" -Name "btnO365" -BackColor ([System.Drawing.Color]::FromArgb(60,70,80)) -ToolTipText "Check Office365 license for a user (MSOnline required)" -Action { Check-O365LicenseStatus }
$btnO365.Location = New-Object System.Drawing.Point(10, $yPosition)
$buttonContainer.Controls.Add($btnO365)
$yPosition += 47

$btnCredDiag = New-Button -Text "Credential Manager Diagnostics" -Name "btnCredDiag" -BackColor ([System.Drawing.Color]::FromArgb(60,70,80)) -ToolTipText "List stored credentials and basic checks" -Action { Show-CredentialManager }
$btnCredDiag.Location = New-Object System.Drawing.Point(10, $yPosition)
$buttonContainer.Controls.Add($btnCredDiag)
$yPosition += 47

$btnADReport = New-Button -Text "AD: Advanced Report" -Name "btnADReport" -BackColor ([System.Drawing.Color]::FromArgb(60,70,80)) -ToolTipText "Generate basic AD summary (requires AD module)" -Action { AD-AdvancedReport }
$btnADReport.Location = New-Object System.Drawing.Point(10, $yPosition)
$buttonContainer.Controls.Add($btnADReport)
$yPosition += 47

$btnRDPTest = New-Button -Text "RDP Troubleshoot" -Name "btnRDPTest" -BackColor ([System.Drawing.Color]::FromArgb(60,70,80)) -ToolTipText "Test RDP service and port" -Action { Test-RemoteDesktop }
$btnRDPTest.Location = New-Object System.Drawing.Point(10, $yPosition)
$buttonContainer.Controls.Add($btnRDPTest)
$yPosition += 47

$btnVPNDiag = New-Button -Text "VPN Diagnostics" -Name "btnVPNDiag" -BackColor ([System.Drawing.Color]::FromArgb(60,70,80)) -ToolTipText "Run VPN related checks and RasMan status" -Action { VPN-Diagnostics }
$btnVPNDiag.Location = New-Object System.Drawing.Point(10, $yPosition)
$buttonContainer.Controls.Add($btnVPNDiag)
$yPosition += 47

$btnEmailTest = New-Button -Text "Email Config Validator" -Name "btnEmailTest" -BackColor ([System.Drawing.Color]::FromArgb(60,70,80)) -ToolTipText "Test mail server connectivity (SMTP/IMAP)" -Action { Test-EmailConfig }
$btnEmailTest.Location = New-Object System.Drawing.Point(10, $yPosition)
$buttonContainer.Controls.Add($btnEmailTest)
$yPosition += 47

$btnBrowserRepair = New-Button -Text "Browser Repair (Cache/Profile)" -Name "btnBrowserRepair" -BackColor ([System.Drawing.Color]::FromArgb(60,70,80)) -ToolTipText "Close browsers and clear caches/profiles (confirm)" -Action { Repair-BrowserProfiles }
$btnBrowserRepair.Location = New-Object System.Drawing.Point(10, $yPosition)
$buttonContainer.Controls.Add($btnBrowserRepair)
$yPosition += 47

$btnExportTicket = New-Button -Text "Export Ticket Documentation" -Name "btnExportTicket" -BackColor ([System.Drawing.Color]::FromArgb(60,70,80)) -ToolTipText "Collect logs and system info into a ticket export on Desktop" -Action { Export-Ticket }
$btnExportTicket.Location = New-Object System.Drawing.Point(10, $yPosition)
$buttonContainer.Controls.Add($btnExportTicket)
$yPosition += 47

# Continue to existing LOG MANAGEMENT section
# MSP Categories & Script Launcher
$buttonContainer.Controls.Add((New-SectionHeader "MSP Categories"))
$yPosition += 30

# Ensure tools directory exists
function Ensure-ToolsFolder {
    $global:ToolsFolder = Join-Path $env:USERPROFILE 'OneDrive\Desktop\mspscripts'
    if (-not (Test-Path $global:ToolsFolder)) {
        try { New-Item -Path $global:ToolsFolder -ItemType Directory -Force | Out-Null; Log "Created tools folder: $global:ToolsFolder" "INFO" } catch { Log "Failed to create tools folder: $_" "WARN" }
    }
}

# Dialog to list and run scripts from tools folder
function Show-ScriptsDialog {
    param([string]$Title = "Available MSP Scripts")
    Ensure-ToolsFolder
    $scripts = Get-ChildItem -Path $global:ToolsFolder -Filter *.ps1 -ErrorAction SilentlyContinue | Sort-Object Name
    $formS = New-Object System.Windows.Forms.Form
    $formS.Text = $Title
    $formS.Size = New-Object System.Drawing.Size(700,500)
    $formS.StartPosition = 'CenterScreen'
    $formS.BackColor = [System.Drawing.Color]::FromArgb(45,45,45)

    $list = New-Object System.Windows.Forms.ListBox
    $list.Location = New-Object System.Drawing.Point(10,10)
    $list.Size = New-Object System.Drawing.Size(660,380)
    $list.BackColor = [System.Drawing.Color]::FromArgb(60,60,60)
    $list.ForeColor = [System.Drawing.Color]::Gainsboro
    $list.SelectionMode = 'One'
    foreach ($s in $scripts) { $null = $list.Items.Add($s.Name) }
    $formS.Controls.Add($list)

    $runBtn = New-Object System.Windows.Forms.Button
    $runBtn.Text = "Run Selected Script"
    $runBtn.Location = New-Object System.Drawing.Point(10,400)
    $runBtn.Size = New-Object System.Drawing.Size(160,30)
    $runBtn.BackColor = [System.Drawing.Color]::FromArgb(60,70,80)
    $runBtn.ForeColor = [System.Drawing.Color]::Gainsboro
    $runBtn.Add_Click({
        if ($list.SelectedItem -eq $null) { [System.Windows.Forms.MessageBox]::Show($formS, "Select a script first","Run Script", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information); return }
        $scriptPath = Join-Path $global:ToolsFolder $list.SelectedItem
        if (-not (Test-Path $scriptPath)) { Log "Script not found: $scriptPath" "WARN"; return }
        if (-not (Show-Confirmation -Message "Run script: $($list.SelectedItem)?")) { return }
        try {
            Log ("Running script: {0}" -f $list.SelectedItem) "INFO"
            Start-Process -FilePath 'powershell' -ArgumentList '-NoProfile','-ExecutionPolicy','Bypass','-File',"`"$scriptPath`"" -Wait -NoNewWindow
            Log ("Script completed: {0}" -f $list.SelectedItem) "PASS"
        } catch {
            Log ("Script run error: {0} - {1}" -f $list.SelectedItem, $_) "FAIL"
        }
    })
    $formS.Controls.Add($runBtn)

    $openBtn = New-Object System.Windows.Forms.Button
    $openBtn.Text = "Open Tools Folder"
    $openBtn.Location = New-Object System.Drawing.Point(180,400)
    $openBtn.Size = New-Object System.Drawing.Size(160,30)
    $openBtn.BackColor = [System.Drawing.Color]::FromArgb(75,85,95)
    $openBtn.ForeColor = [System.Drawing.Color]::Gainsboro
    $openBtn.Add_Click({ Start-Process -FilePath $global:ToolsFolder })
    $formS.Controls.Add($openBtn)

    $closeBtn = New-Object System.Windows.Forms.Button
    $closeBtn.Text = "Close"
    $closeBtn.Location = New-Object System.Drawing.Point(520,400)
    $closeBtn.Size = New-Object System.Drawing.Size(150,30)
    $closeBtn.Add_Click({ $formS.Close() })
    $formS.Controls.Add($closeBtn)

    $formS.ShowDialog() | Out-Null
}

# Category buttons (open scripts dialog filtered by folder)
$btnNetCategory = New-Button -Text "Network Troubleshooting" -Name "btnNetCategory" -BackColor ([System.Drawing.Color]::FromArgb(60,70,80)) -ToolTipText "Network troubleshooting scripts and tools" -Action { Show-ScriptsDialog -Title 'Network Scripts' }
$btnNetCategory.Location = New-Object System.Drawing.Point(10, $yPosition)
$buttonContainer.Controls.Add($btnNetCategory)
$yPosition += 47

$btnADCategory = New-Button -Text "Active Directory" -Name "btnADCategory" -BackColor ([System.Drawing.Color]::FromArgb(60,70,80)) -ToolTipText "AD management scripts" -Action { Show-ScriptsDialog -Title 'Active Directory Scripts' }
$btnADCategory.Location = New-Object System.Drawing.Point(10, $yPosition)
$buttonContainer.Controls.Add($btnADCategory)
$yPosition += 47

$btnAzureCategory = New-Button -Text "Azure / M365 / Intune" -Name "btnAzureCategory" -BackColor ([System.Drawing.Color]::FromArgb(60,70,80)) -ToolTipText "Azure and M365 tools" -Action { Show-ScriptsDialog -Title 'Azure & M365 Scripts' }
$btnAzureCategory.Location = New-Object System.Drawing.Point(10, $yPosition)
$buttonContainer.Controls.Add($btnAzureCategory)
$yPosition += 47

$btnExchangeCategory = New-Button -Text "Exchange / Email" -Name "btnExchangeCategory" -BackColor ([System.Drawing.Color]::FromArgb(60,70,80)) -ToolTipText "Exchange and email tools" -Action { Show-ScriptsDialog -Title 'Exchange Scripts' }
$btnExchangeCategory.Location = New-Object System.Drawing.Point(10, $yPosition)
$buttonContainer.Controls.Add($btnExchangeCategory)
$yPosition += 47

$btnPrinterCategory = New-Button -Text "Printers & Devices" -Name "btnPrinterCategory" -BackColor ([System.Drawing.Color]::FromArgb(60,70,80)) -ToolTipText "Printer and device tools" -Action { Show-ScriptsDialog -Title 'Printer Scripts' }
$btnPrinterCategory.Location = New-Object System.Drawing.Point(10, $yPosition)
$buttonContainer.Controls.Add($btnPrinterCategory)
$yPosition += 47

$btnBrowserCategory = New-Button -Text "Browsers & Profiles" -Name "btnBrowserCategory" -BackColor ([System.Drawing.Color]::FromArgb(60,70,80)) -ToolTipText "Browser cache and profile repair" -Action { Show-ScriptsDialog -Title 'Browser Scripts' }
$btnBrowserCategory.Location = New-Object System.Drawing.Point(10, $yPosition)
$buttonContainer.Controls.Add($btnBrowserCategory)
$yPosition += 47

$btnAutomationCategory = New-Button -Text "Automation & Templates" -Name "btnAutomationCategory" -BackColor ([System.Drawing.Color]::FromArgb(60,70,80)) -ToolTipText "Automation scripts, templates and runbooks" -Action { Show-ScriptsDialog -Title 'Automation Scripts' }
$btnAutomationCategory.Location = New-Object System.Drawing.Point(10, $yPosition)
$buttonContainer.Controls.Add($btnAutomationCategory)
$yPosition += 47

$btnSecurityCategory = New-Button -Text "Security & Phishing" -Name "btnSecurityCategory" -BackColor ([System.Drawing.Color]::FromArgb(60,70,80)) -ToolTipText "Security, phishing and baseline checks" -Action { Show-ScriptsDialog -Title 'Security Scripts' }
$btnSecurityCategory.Location = New-Object System.Drawing.Point(10, $yPosition)
$buttonContainer.Controls.Add($btnSecurityCategory)
$yPosition += 47

# Continue to LOG MANAGEMENT section
...
# OS Detection with fallbacks
try {
    $osInfo = Get-WmiObject Win32_OperatingSystem -ErrorAction Stop
        Log ("OS Version: {0} (Build {1})" -f $osInfo.Caption, $osInfo.BuildNumber) "INFO"
} catch {
    try {
        $osVersion = (Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion").ProductName
            Log ("OS Version: {0} (Fallback detection)" -f $osVersion) "INFO"
    } catch {
        Log "Could not determine OS version" "WARN"
    }
}

Log "Toolkit loaded with $($buttonContainer.Controls.Count) tools" "PASS"
