...
<#
.SYNOPSIS
MSP Technician Toolkit — v2.5.3 snapshot
.DESCRIPTION
Toolkit snapshot for v2.5.3 with compatibility and printer improvements.
.NOTES
...
# Include release notes for this snapshot (from change_2.5.3.ps1)
if (Test-Path (Join-Path $PSScriptRoot 'change_2.5.3.ps1')) { . (Join-Path $PSScriptRoot 'change_2.5.3.ps1') }
...
$form.Text = "MSP Technician IDE - v2.5.3 (Running as Admin)"
...
# Include v2 sections and compatibility fixes such as ghost printer removal and pending reboot detection
Add-SectionLabel "PRINTER TROUBLESHOOTING"
Create-Btn "Remove Ghost Printers" "LightCoral" {
	Log "Removing Ghost Printers..." "WARN"
	# attempt to remove ghost/offline printers via WMI
	$printers = Get-WmiObject -Class Win32_Printer | Where-Object { $_.WorkOffline -eq $true }
	foreach($p in $printers){
		try { (New-Object -ComObject WScript.Network).RemovePrinterConnection($p.Name); Log "Attempted removal: $($p.Name)" "PASS" }
		catch { Log "Could not remove $($p.Name)" "WARN" }
	}
}

# (rest of UI mirrors v2.0 features)
